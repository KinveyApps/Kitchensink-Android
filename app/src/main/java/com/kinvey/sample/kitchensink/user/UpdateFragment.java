...
public class UpdateFragment extends UseCaseFragment implements View.OnClickListener{
    ...
    private void performUserUpdate(){
        User user = getClient().getActiveUser();
        user.put("last_name", "Smith");
        user.update(new KinveyUserCallback<User>() {
            @Override
            public void onSuccess(User result) {
                Toast.makeText(getActivity(), "updated user!", Toast.LENGTH_SHORT).show();
            }
            @Override
            public void onFailure(Throwable error) {
                Toast.makeText(getActivity(), "update failed -> " + error.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void lookupAndUpdate() {
        ...
        users.lookup(criteria, new KinveyUserListCallback() {
            @Override
            public void onSuccess(User[] result) {
                Toast.makeText(getActivity(), "lookup returned: " + result.length + " user(s)!", Toast.LENGTH_SHORT).show();
            ...
            @Override
            public void onFailure(Throwable error) {
                Toast.makeText(getActivity(), "lookup failed -> " + error.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
