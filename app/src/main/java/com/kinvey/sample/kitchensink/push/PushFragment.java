...
public class PushFragment extends UseCaseFragment implements View.OnClickListener {
    ...
    public void bindViews(View v) {
        ...
        pushStatus.setText(Boolean.toString(isPushEnabled()));
    }

    private boolean isPushEnabled() {
        return getClient().push(FCMService.class).isPushEnabled();
    }

    private void initPush() {
        getClient().push(FCMService.class).initialize(App.getInstance());
    }

    @Override
    public void onClick(View v) {
        if (v == bRegister) { registerPush(); }
    }
