...
public class PutFragment extends UseCaseFragment implements View.OnClickListener {
    ...
    private void deleteAll(){
		 ...
		 */
        Query q = new Query();
        getApplicationContext().getClient().appData(KitchenSink.collectionName, MyEntity.class).delete(q, new KinveyDeleteCallback() {
            @Override
            public void onSuccess(KinveyDeleteResponse result) {
                AndroidUtil.toast(getActivity(), "deleted " + result.getCount() + "entities!");
                getCount();
            }
